...
The full-stack launcher and the individual web/API/knowledge workflows must not run at the same time when they target the same fixed ports.
...
**How to apply:** Before verifying or restarting the full-stack launcher, stop or avoid the individual workflows for the same ports; otherwise verify the individual artifact workflows independently.