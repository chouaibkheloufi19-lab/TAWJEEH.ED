...
**How to apply:** Check `listArtifacts()` after a transition; if the imported web artifact is absent, register it before trying to present or screenshot the app. Keep the generated artifact service metadata, restore the imported source, then perform a package install scoped to that workspace package before restarting its managed workflow.