...
- Required setup: Replit PostgreSQL (`DATABASE_URL` is managed automatically), either `DEEPSEEK_API_KEY` for server-side DeepSeek text generation or a working Replit-managed xAI connection for Grok text generation, and Replit-managed Clerk Auth
- Optional env: `KNOWLEDGE_BASE_URL` (defaults to `http://127.0.0.1:8001/knowledge`), `DEEPSEEK_MODEL` (defaults to `deepseek-chat`), `XAI_MODEL` or `GROK_TEXT_MODEL` to pin a text model, and `GROK_VISION_MODEL` for the optional image-analysis path
